...
function getChartPropsFromContent(content, isYaml, chartJsDefaults) {
  ...
  return {
    ...
    redraw:
      chartProps.redraw !== undefined
        ? chartProps.redraw
        : chartJsDefaults.redraw,
    datasetIdKey:
      chartProps.datasetIdKey !== undefined
        ? chartProps.datasetIdKey
        : chartJsDefaults.datasetIdKey,
    updateMode:
      chartProps.updateMode !== undefined
        ? chartProps.updateMode
        : chartJsDefaults.updateMode
  };
}
...
function getImportStatements(imports) {
  ...
  const importString = Object.entries(imports)
    .map((entry) => {
      return imports[entry[0]] ? entry[0] : undefined;
    })
    .filter(Boolean)
    .join(", ");
...
function createChartNode(chartJs) {
  const redraw =
    chartJs.redraw !== undefined
      ? `redraw={${chartJs.redraw ? "true" : "false"}}`
      : "";
  const datasetIdKey =
    chartJs.datasetIdKey !== undefined
      ? `datasetIdKey="${chartJs.datasetIdKey}"`
      : "";
  const updateMode =
    chartJs.updateMode !== undefined
      ? `updateMode="${chartJs.updateMode}"`
      : "";
